          ...
          {faculties.length > 0 ? (
            <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              {faculties.map((faculty) => (
                <li
                  key={faculty.id}
                  className="bg-white border border-gray-200 rounded-2xl shadow hover:shadow-lg transform hover:-translate-y-1 transition duration-300 p-5 flex flex-col"
                >
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="text-lg font-bold text-[#2F4F83]">
                      {faculty.code}
                    </h3>
                  </div>
                  <p className="text-gray-700 font-medium mb-2">{faculty.name}</p>
